...
export default function Accessories() {
  ...
  useEffect(() => {
    const fetchAccessories = async () => {
      try {
        const response = await axios.get('https://quirko-e-commerce.onrender.com/getallaccessories');
        setAccessories(response.data);
        setLoading(false);
      } catch (error) {
        setError('Error fetching accessories: ' + error.message);
        setLoading(false);
      }
    };
  ...
  const addToCart = async (id) => {
    try {
      const selectedAccessory = accessories.find((accessory) => accessory.id === id);
      if (selectedAccessory) {
        const response = await axios.post('https://quirko-e-commerce.onrender.com/addcart', selectedAccessory);
        if (response.status === 200) {
          setCartItems([...cartItems, selectedAccessory]);
          console.log('Item added to cart:', selectedAccessory);
        } else {
  ...
  return (
    <div className="bg-gradient-to-br from-[#055B5C] via-green-500 to-[#018C85] animate-gradient-x">
      <div className="container mx-auto py-8">
        <h1 className="text-2xl font-bold text-white mb-4 p-4">Accessories List</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 px-4 pb-8">
          {accessories.map((accessory) => (
            <div key={accessory.id} className="card-wrapper">
              <div className="bg-white border-4 border-gray-200 p-4 rounded-lg shadow-md transition duration-300 transform hover:scale-105 hover:shadow-lg">
                ...
                <div className="mt-4">
                  <h2 className="text-xl font-semibold font-times">{accessory.accessory_name}</h2>
                  <p className="text-gray-700 font-times">{accessory.description}</p>
                  <p className="mt-2 font-times">Rating: {accessory.rating}</p>
                  <p className="mt-2 font-times">Price: {accessory.price}</p>
                </div>
              </div>
            </div>
          ))}

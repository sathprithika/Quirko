  ...
  useEffect(() => {
    const fetchArts = async () => {
      try {
        ...
        setError('Error fetching arts: ' + error.message);
        setLoading(false);
      }
    };
  ...
  return (
    <div className="bg-gradient-to-br from-[#055B5C] via-green-500 to-[#018C85] animate-gradient-x">
      <div className="container mx-auto py-8">
        <h1 className="text-2xl font-bold text-white mb-4 p-4">Art Works List</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 px-4 pb-8">
          {arts.map((art) => (
            <div key={art.id} className="card-wrapper">
              <div className="bg-white border-4 border-gray-200 p-4 rounded-lg shadow-md transition duration-300 transform hover:scale-105 hover:shadow-lg">
                ...
                <div className="mt-4">
                  <h2 className="text-xl font-semibold font-times">{art.art_name}</h2>
                  <p className="text-gray-700 font-times">{art.description}</p>
                  <p className="mt-2 font-times">Rating: {art.rating}</p>
                  <p className="mt-2 font-times">Price: {art.price}</p>
                </div>
              </div>
            </div>
          ))}
